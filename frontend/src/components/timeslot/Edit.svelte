<script lang="ts">
    ...
    let originalJob: string | null = null;
    let timeslot: UpdateTimeslot | null;
    let commentRows: number;

    export function edit(t: Timeslot, job: string | null) {
        originalJob = job;
        originalTimeslot = t;
        timeslot = {
...
{#if timeslot != null}
    ...
    >
        <div class="flex flex-col space-y-6">
            ...
            <div class="relative">
                <Label class="absolute text-sm -translate-y-4 scale-75 z-10 origin-left rtl:origin-right px-2 peer-focus:px-2 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 start-1 text-gray-500 dark:text-gray-400 peer-focus:text-primary-600 peer-focus:dark:text-primary-500 top-2 cursor-text bg-white dark:bg-gray-800">Comment</Label>
                <Textarea
