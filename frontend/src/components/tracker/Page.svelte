<script lang="ts">
    ...
    import { browserLanguage, DisplayRangeUnit, formatDuration, getWeekNumber, getYearFromWeek, MouseClick, TimePensumUnit, Timeslot, timezone, Tracker, type Unit } from "../../app";
        ...
        if (fetched || force) {
            ...
            if (res.ok) {
                ...
                for (let unit of slots) {
                    unit.timeslots = unit.timeslots.map(timeslot => 
                        Object.assign(new Timeslot(), timeslot)
                    );
                }
                timeslots = slots;
            } else {
    ...
    let fields: [
        string,
        (t: Timeslot) => string | Writable<string>,
        ((t: Timeslot, mouseClick: MouseClick) => void) | null,
        boolean
    ][] = [
        ["Date", (t) => {
            ...
            if (end != null && (start.getFullYear() != end.getFullYear() || start.getMonth() != end.getMonth() || start.getDate() != end.getDate())) {
                if ((tracker?.timePensumUnit as any) == TimePensumUnit[TimePensumUnit.Month]) return toString(start) + "-" + toString(end);
    ...
    function openEdit() {
        editTracker.$$.ctx[editTracker.$$.props.edit](tracker);
    }
    ...
    function getWeekDates(unit: Unit, unitIdx: number): string {
        ...
        function displayDate(date: Date,): string {
            return date.toLocaleString(browserLanguage, {year: "numeric", month: "numeric", day: "numeric"})
        }
...
{#if tracker != null}
    ...
    <div class="mb-8 mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <SmallCard>
            {#if tracker.timePensum}
                Time pensum: <span class="float-right">
                    {formatDuration(tracker.timePensum * 60, true)}{tracker.timePensumUnit?" / " + tracker.timePensumUnit:""}
                </span>
        ...
        <SmallCard>
            Time worked: <span class="float-right font-mono" id="time-worked">
                {#if timeslots}
                    <TimeWorked timeslots={timeslots} filterUnit={$timeslotsUnit} filterPrimary={$timeslotsPrimary} filterSecondary={$timeslotsSecondary}/>
                {/if}
            </span>
        </SmallCard>
        <SmallCard>
    ...
    <div class="flex flex-row gap-2">
        {#each Object.keys(DisplayRangeUnit).filter(key => isNaN(Number(key))) as displayRangeUnit}
            <A asButton color={$timeslotsUnit == displayRangeUnit ? "text-primary-600 dark:text-primary-500" : "text-secondary-600 dark:text-secondary-500"} on:click={() => timeslotsUnit.set(displayRangeUnit)}>{displayRangeUnit}</A>
        {/each}
    </div>
    <div class="flex flex-row gap-2 flex-wrap justify-items-stretch md:justify-items-start">
        <div class="grow md:grow-0">
            <Label class={$timeslotsUnit != DisplayRangeUnit[DisplayRangeUnit.Year]?"mt-2":"hidden"}>
                Year:
            </Label>
            <Select class={$timeslotsUnit != DisplayRangeUnit[DisplayRangeUnit.Year]?"":"mt-2"} items={timeslotsPrimaryOptions} bind:value={$timeslotsPrimary} />
        </div>
    ...
    {#if timeslots != null}
        ...
        >
            {#each timeslots as unit, i}
                {@const shouldShow = i == 0 || unit.timeslots.length != 0 || (unit.timeslots.length == 0 && timeslots[i-1].timeslots.length != 0)}
                {#if shouldShow}
                    <thead>
                        <tr>
                            <th colspan="{fields.length+1}">
                                <Heading tag="h2" class="mt-6 mb-4 inline-block w-auto text-xl">{getUnitTitle(unit, i, $timeslotsUnit)}
    ...
    <Delete
        ...
        entity={tracker}
        on:deleted={() => (window.location.href = "/trackers")}
    />

    <Delete bind:isOpen={isDeleteTimeslotOpen} entity={deleteTimeslot} on:deleted={(e) => {
    ...
    <Create api={getRoute() + "/timeslots"} bind:isOpen={isCreateOpen} on:created={(e) => {
        let start = new Date(e.detail.start);
        let startUnit = new Date(start.getFullYear() + "-" + String(start.getMonth() + 1).padStart(2, "0") + "-" + String(start.getDate()).padStart(2, "0"));
        for (let unit of timeslots || []) {
    ...
    <EditTimeslot bind:this={editTimeslot} on:update={(e) => {
        for (let unit of timeslots || []) {
            for (let i = 0; i < unit.timeslots.length; i += 1) {
                if (unit.timeslots[i].id == e.detail.id) {
